					...
					on: {
						onChange: (id) => {
							...
							if (table && table.name !== "clinical metadata") {
								webix.ajax(`${webix.serverURL}/tcga/case/${caseId}/metadata/${table.name}`).then(data => data.json())
									.then((data) => {
						...
						onAfterRender: () => {
							...
							comboList.clearAll();
							if (this.currentSlide && this.currentSlide.meta && this.currentSlide.meta.clinicalMetaData) {
