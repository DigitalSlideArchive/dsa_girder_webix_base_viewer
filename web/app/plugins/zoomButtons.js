function create_fixed_zoom_btn(viewer, helper, zb) {
    var url_base = "/app/static/openseadragon/images/";
    var btn = new OpenSeadragon.Button({
                ...
                onClick: function() {
                    ...
                    // //console.log("zoomVal="+newzoom); helper.setZoomFactor(newzoom); //viewer.viewport.zoomTo(zoomVal, null, true);
                    //viewer.viewport.applyConstraints(); return false; } }); 
                    return btn;
                }
                var imagingHelper = new OpenSeadragonImaging.ImagingHelper({ viewer: seadragon });
                ...
                var btn_list = [];
                for (var i = 0; i < zoom_btns.length; i++) { var zb = zoom_btns[i]; var btn = create_fixed_zoom_btn(seadragon, imagingHelper, zb);
                    btn_list.push(btn); }
                var btnGroup = new OpenSeadragon.ButtonGroup({ buttons: btn_list });
                seadragon.addControl(btnGroup.element, { attachToViewer: true, anchor: OpenSeadragon.ControlAnchor.TOP_LEFT });